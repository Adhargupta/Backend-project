...
const userSchema = new Schema(
    ...
    refreshToken:{                                          // refresh tokens used to generate new access token (access tokaens are short lived and refresh tokens are long lived) and we will store refresh token in database to verify it when user sends request with refresh token to generate new access token
        type:String,
    }
...
//     if(!this.isModified("password")) return next();                                              // this is used to check if password is modified (modified is in built) or not
...
userSchema.pre("save", async function () {
    ...
    if (!this.isModified("password")) return;                                   // checks: Has the password field changed in this document since it was loaded or created?

    this.password = await bcrypt.hash(this.password, 10);

});

userSchema.methods.isPasswordCorrect = async function(password){                 // this is used to compare the password entered by user with the password stored in database and return true pr false
